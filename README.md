...
- `/parse-task` には小さなレート制限を入れています
- ログには生の入力全文を残しません
- OpenAI が失敗しても、rule-based parser と fallback で結果を返す設計です

## Portfolio Scope

- このプロジェクトは就活用ポートフォリオとして作成しています
- 主にローカル実行、README、スクリーンショットでの説明を想定しています
- 入力は短い単一タスクを想定し、`150文字` までに制限しています
...
- 単一タスク入力を想定しています
- 日付処理は限定的です
- rule-based parser は、代表ケースの再現性とフォールバックの安定性を重視した実装です。幅広い自然文への汎用対応は主に OpenAI 側を想定しており、rule-based 側は壊れにくい最小構成として位置づけています。
...
このリポジトリでは、ローカル実行前提の小さな安全策として入力制限やレート制限も実装しています。
